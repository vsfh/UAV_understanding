...
\input{table/dataset_comparison}
...
For a verified pair $(x^g,x^l,y)$, a Qwen3.6-35B-A3B-FP8 teacher VLM~\cite{qwen2026qwen36} receives the ordered wide context and linked crop and first produces a label-agnostic description $c^{(0)}$. The teacher is not shown $y$, filenames, or ontology text in this pass. Greedy decoding returns a fixed JSON object containing one concise description, separate context and crop observations, cross-view relations, and visibly unresolved details. A constrained second pass receives the same images, $c^{(0)}$, the verified event definition and factors, and one confusion neighbor $y^{-}$ selected by a deterministic hash of the record identifier. It emits the exact student target
\[
  t=(\texttt{events},\texttt{factors},\texttt{evidence},\texttt{uncertain})
\]
and a separate counterfactual statement for $y^{-}$. The rewrite must copy verified factors exactly, name $y$ in readable form, cite only visible evidence, distinguish $y^{-}$, and set uncertainty when support is incomplete. The current development ontology contains compact definitions and confusion edges but no adjudicated per-item factors; consequently the development generator supplies an empty factor map rather than inferring one, while the same code accepts versioned evidence cards when available.

A third, label-blind verification pass independently scores literal image--text support for the grounded and counterfactual statements. Deterministic gates require the event phrase and confusion distinction, exact factor agreement, no unsupported named entity, positive entailment of at least $0.65$ with no unsupported claim, and counterfactual entailment below $0.50$. To prevent a generative teacher from weakening a negative with a denial or disclaimer, the positive assertion for $y^{-}$ is instantiated deterministically from its frozen ontology definition; the teacher generates $c$ and the distinction, while the verifier tests both $c$ and this fixed counterfactual. Thus the later unlikelihood loss penalizes the neighbor claim rather than a correct rejection explanation. Verifier reasons are also schema-checked independently and may not use cross-candidate meta-commentary. Schema or gate failures are retained for review, never silently repaired into accepted supervision. All prompts use the versioned \texttt{clear-uav-teacher-v5} template, at most 262,144 pixels per image, disabled thinking traces, strict JSON parsing, and resumable per-record caches. The first-pass description defines the generic-caption baseline; the constrained statement defines grounded-caption supervision.

Every generated row is marked \texttt{pending\_human\_review} regardless of its automatic score. Human review covers every core test item, every rare/tail item used for evaluation, and a stratified sample of training captions; the official full-supervision runner additionally requires every selected training target and counterfactual used by its loss to carry the \texttt{human\_audited} tier. We report acceptance, correction, and counterfactual-susceptibility rates rather than treating synthetic text as ground truth by default.
