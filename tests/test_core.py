...
def test_suite_summary_mean_std_and_tex(tmp_path: Path) -> None:
    steps = []
    for seed, macro_f1 in ((42, 0.6), (43, 0.8)):
        metric_path = tmp_path / f"seed{seed}.json"
        metric_path.write_text(
            json.dumps({"macro_f1": macro_f1, "micro_f1": 0.9}),
            encoding="utf-8",
        )
        steps.append(
            {
                "step_id": f"analysis-{seed}",
                "kind": "analysis",
                "command": [],
                "outputs": [str(metric_path)],
                "metadata": {
                    "protocol": "session_disjoint",
                    "experiment": "label_pair",
                    "seed": seed,
                    "split": "val",
                    "decoder": "closed",
                },
    ...
    tex_output = tmp_path / "summary.tex"
    subprocess.run(
        [
            sys.executable,
            ROOT / "scripts/summarize_suite.py",
            "--plan",
            plan,
            "--output",
            output,
    ...
    assert summary["metrics"]["macro_f1"]["mean"] == 0.7
    assert summary["num_seeds"] == 2
    assert "70.0 $\\pm$ 14.1" in tex_output.read_text(encoding="utf-8")
